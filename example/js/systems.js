// System-specific content and functionality
const systemsData = {
    skin: {
        title: "ระบบผิวหนัง",
        description:
...
function getSystemContent(systemType) {
    ...
    if (!system) return "<p>ไม่พบข้อมูลระบบนี้</p>";
    ...
    return `
        <div class="system-modal-content">
                ...
                <div class="system-visual">
                    <div class="system-diagram" data-system="${systemType}">
                        ...
                        <div class="diagram-controls">
                            <button class="zoom-in" onclick="zoomDiagram(1.2)">
                                <i class="fas fa-search-plus"></i>
                            </button>
                            <button class="zoom-out" onclick="zoomDiagram(0.8)">
            ...
            <div class="system-footer">
                <button class="back-to-systems" onclick="closeSystemModal()">
...
function initSystemInteractions(systemType) {
    ...
    // Add click interactions to diagram parts
    addDiagramClickHandlers(systemType);

    // Initialize quiz interactions
    initQuizInteractions(systemType);
}

function addDiagramClickHandlers(systemType) {
    // This would be expanded with specific interactive elements for each system
    const diagram = document.querySelector(".system-diagram");
    if (diagram) {
        diagram.addEventListener("click", (e) => {
            // Add interactive hotspots for different parts of the diagram
            showDiagramTooltip(e, systemType);
        });
    }
}

function showDiagramTooltip(event, systemType) {
    // Create tooltip with information about clicked area
    const tooltip = document.createElement("div");
    tooltip.className = "diagram-tooltip";
    tooltip.innerHTML = "คลิกที่ส่วนต่างๆ เพื่อดูรายละเอียด";
...
function checkQuizAnswers(systemType) {
    ...
    // Show results
    const resultsDiv = document.getElementById(`quiz-results-${systemType}`);
    ...
    // Show reset button
    document.querySelector(".reset-quiz").style.display = "inline-block";
...
function resetQuiz(systemType) {
    ...
    // Clear results
    document.getElementById(`quiz-results-${systemType}`).innerHTML = "";

    // Show check button, hide reset button
    document.querySelector(".check-answers").style.display = "inline-block";
...
function closeSystemModal() {
    // Use the same helper function from main.js
    if (typeof closeModal === "function") {
        closeModal();
    } else {
        // Fallback if closeModal function is not available

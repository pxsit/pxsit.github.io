  ...
  function render() {
    ...
    if (i >= currentQuestions.length) {
      ...
      const summaryItems = answersLog
        .map((entry, idx) => {
          const wasSkipped = entry.selectedIndex === null || entry.selectedIndex === undefined;
          const isCorrect = !wasSkipped && entry.selectedIndex === entry.correctIndex;
          const choiceHtml = entry.choices
            .map((c, cIdx) => {
              ...
              if (!isCorrect && !wasSkipped && cIdx === entry.selectedIndex) cls += " incorrect";
              // keep selected style for visibility (optional)
              if (!wasSkipped && cIdx === entry.selectedIndex) cls += " selected";
          ...
          return `
            <div class="quiz-summary-item" style="margin:16px 0; padding:12px; border-radius:12px; background: rgba(0,0,0,0.25);">
              <div style="display:flex; justify-content:space-between; align-items:center; gap:8px; flex-wrap:wrap;">
                ...
                <span class="muted">${isCorrect ? "ตอบถูก" : wasSkipped ? "ข้าม" : "ตอบผิด"}</span>
              </div>
